...
class ApiClient {
  ...
  private async handleResponse<T>(response: Response): Promise<T> {
    ...
    if (!response.ok) {
      ...
      console.error(
        `[API] ${response.status} Error Data:`,
  ...
  private async request<T>(
    ...
    if (isFormData) {
      ...
      console.log(
        `[API] ${method} Request with FormData (Content-Type deleted for fetch)`,
      );
    } else {
      console.log(
        `[API] ${method} Request with Headers:`,
        JSON.stringify(Object.fromEntries(headers as any), null, 2),
      );
    }
  ...
  /**
   * Specialized UPLOAD request using XMLHttpRequest for progress tracking
   */
  async upload<T>(
    endpoint: string,
    formData: FormData,
    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.onerror = () => {
        reject({ message: "Network request failed", status: 0 });
      };
