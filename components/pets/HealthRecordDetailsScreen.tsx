...
export default function HealthRecordDetailsScreen({ pet, record }: Props) {
    ...
    const getIconForType = (type: string) => {
        switch (type?.toLowerCase()) {
            case 'vaccination': return InjectionIcon;
            case 'checkup': return Stethoscope02Icon;
            case 'medication': return Medicine01Icon;
            case 'tick': return Bug02Icon;
            case 'surgery': return BandageIcon;
            case 'dental': return DentalToothIcon;
            default: return Note01Icon;
        }
    };
        ...
        <SafeAreaView style={styles.safeArea} edges={['right', 'left', 'bottom']}>
            ...
            <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.card}>
                    ...
                    <View style={styles.vetHeader}>
                        <Text style={styles.vetName}>Dr. {record.vetName || 'Unknown'}</Text>
                    ...
                    {record.observations && record.observations.length > 0 ? (
                        record.observations.map((obs, index) => (
                            <View key={index} style={styles.observationItem}>
                                <HugeiconsIcon icon={ViewIcon} size={16} color={Colors.textSecondary} />
            ...
            {viewedImage && (
                <View style={styles.previewOverlay}>
                    <TouchableOpacity style={styles.closePreview} onPress={() => setViewedImage(null)}>
